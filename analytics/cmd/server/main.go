...
import (
	...
	"github.com/labbrly/analytics/internal/analytics"
...
func main() {
	...
	eventsCol := mongoClient.Database("analytics").Collection("events")
	rdb := redisutil.Connect()

	store := analytics.New(rdb, eventsCol)
	...
	go func() {
		slog.Info("analytics service starting", "addr", srv.Addr)
